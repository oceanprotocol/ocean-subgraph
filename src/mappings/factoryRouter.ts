import { NewPool, TokenAdded, OPCFeeChanged } from '../@types/FactoryRouter/FactoryRouter'
import { BigInt } from '@graphprotocol/graph-ts'
import { Pool } from '../@types/schema'
import { BPool } from '../@types/templates'
import { addPool } from './utils/globalUtils'
import { getOPC } from './utils/globalUtils'
import { weiToDecimal } from './utils/generic'
import { FactoryRouter, FactoryRouter__getOPCFeesResult } from '../@types/FactoryRouter/FactoryRouter'
...
export function handleOPCFeeChanged(event: OPCFeeChanged): void {
  ...
  opc.swapOceanFee = weiToDecimal(event.params.newSwapOceanFee.toBigDecimal(), decimals)
  opc.swapNonOceanFee = weiToDecimal(event.params.newSwapNonOceanFee.toBigDecimal(), decimals)
  opc.consumeFee = weiToDecimal(event.params.newConsumeFee.toBigDecimal(), decimals)
  opc.providerFee = weiToDecimal(event.params.newProviderFee.toBigDecimal(), decimals)
  opc.save()
}
...
export function handleTokenAdded(event: TokenAdded): void {
  ...
  const opc = getOPC()
  const decimals = BigInt.fromI32(18).toI32()
  opc.swapOceanFee = weiToDecimal(oceanFees.value.value0.toBigDecimal(), decimals)
  opc.swapNonOceanFee = weiToDecimal(oceanFees.value.value1.toBigDecimal(), decimals)
