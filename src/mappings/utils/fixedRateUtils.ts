...
export function getFixedRateGraphID(exchangeId: string, contractAddress: Address): string {
...
export function updateFixedRateExchangeSupply(exchangeId: Bytes, contractAddress: Address): void{
  const fixedRateID=contractAddress.toHexString() + "-"+ exchangeId.toHexString()
  ...
  fixedRateExchange.datatokenBalance = weiToDecimal(fixedRateDetails.value.value9.toBigDecimal(),datatoken.decimals)
  fixedRateExchange.baseTokenBalance = weiToDecimal(fixedRateDetails.value.value10.toBigDecimal(),baseToken.decimals)
  fixedRateExchange.datatokenSupply = weiToDecimal(fixedRateDetails.value.value7.toBigDecimal(),datatoken.decimals)
  fixedRateExchange.baseTokenSupply = weiToDecimal(fixedRateDetails.value.value8.toBigDecimal(),baseToken.decimals)
