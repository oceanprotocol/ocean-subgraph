...
describe('User tests', async () => {
  ...
  it('Deploying a Fixed Rate Exchange & Test User Fields', async () => {
    ...
    assert(user.tokenBalancesOwned.length === 0, 'incorrect value for: id')
    assert(user.orders.length === 0, 'incorrect value for: id')
    assert(user.freSwaps.length === 0, 'incorrect value for: id')
    assert(user.totalOrders === '0', 'incorrect value for: id')
    assert(user.totalSales === '0', 'incorrect value for: id')
    assert(user.__typename === 'User', 'incorrect value for: id')
  })
